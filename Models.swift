...
// MARK: - New Data Structures for Pokémon-Style Prompt Pipeline

/// Data parsed from the initial "analyse" step of the new prompt pipeline.
/// Expected JSON from OpenAI (based on AnalysePrompt.text):
/// {
///   "subject": "string",
///   "visualTraits": "string",
///   "type": "string" (e.g., "Fire", "Water" - matches CardType.RawValue),
///   "strength": integer,
///   "stamina": integer,
///   "agility": integer
/// }
struct InitialAnalysisData: Decodable {
    let subject: String
    let visualTraits: String
    let type: String // This should match one of CardType.RawValue
    let strength: Int
    let stamina: Int
    let agility: Int
}

/// For decoding the "stats" array from the JSON string generated by CardRecipe.statsJSON.
/// e.g., { "category": "Type", "value": "Fire" }
struct DecodedStatItem: Decodable {
    let category: String
    let value: String // Value is always a string from this prompt, will be converted to StatValue later
}

/// For decoding the overall JSON structure from CardRecipe.statsJSON.
/// e.g., { "stats": [ ...DecodedStatItem... ] }
struct StatsContainer: Decodable {
    let stats: [DecodedStatItem]
}
...
enum SampleCardData {
    static let vansOldSkool = CardContent(
        ...
        detailedSubjectDescription: "A classic pair of Vans Old Skool sneakers, black with a white side stripe.", // Placeholder for sample data
